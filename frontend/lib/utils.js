...
export const timeAgo = (dateString) => {
    ...
    const minutes = Math.floor(seconds / 60);
    if(minutes < 60)
        return `${minutes} minute${minutes > 1 ? 's' : ''} ago`; // ✅ added space before "ago"

    const hours = Math.floor(minutes / 60);
    if(hours < 24)
        return `${hours} hour${hours > 1 ? 's' : ''} ago`; // ✅ Fix 1: was "hour", now "hours"

    const days = Math.floor(hours / 24); // ✅ Fix 2: was "hour/24", now "hours/24"
    if(days < 30)
        return `${days} day${days > 1 ? 's' : ''} ago`; // ✅ removed duplicate 's'

    const months = Math.floor(days / 30);
    if(months < 12)
        return `${months} month${months > 1 ? 's' : ''} ago`; // ✅ removed duplicate 's', added space

    const years = Math.floor(months / 12);
    return `${years} year${years > 1 ? 's' : ''} ago`; // ✅ Fix 3: was "year", now "years"
};

//    Secure Session ID (Better than Math.random)

export const generateSessionId = () => { // ✅ also fixed: was "generatedSessionId"
    if(typeof crypto !== 'undefined' && crypto.randomUUID){
        return crypto.randomUUID();
    }
...
export const getSessionId = () => {
    ...
    if(!id) {
        id = generateSessionId(); // ✅ matches fixed function name above
        sessionStorage.setItem('session_id', id);
    }
    return id;
};
