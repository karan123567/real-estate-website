...
async function apiFetch(endpoint, options = {}) {
  ...
  // ✅ IMPORTANT: Always call Next.js API (proxy)
...
export const propertyAPI = {
  ...
  // ⚠️ Direct backend call (file upload needs it)
  uploadImages: async (files) => {
    const formData = new FormData();
    files.forEach(file => formData.append('images', file));

    return fetch(`${process.env.NEXT_PUBLIC_API_URL}/api/properties/upload/images`, {
      method: 'POST',
      credentials: 'include',
      body: formData,
    }).then(res => res.json());
