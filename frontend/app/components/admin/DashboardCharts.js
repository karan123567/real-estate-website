...
// ✅ Accepts real data as props, shows empty state if no data
export default function DashboardCharts({ visitorsData = [], cityData = [] }) {
  return (
    <div className="grid grid-cols-1 gap-4 lg:grid-cols-3">
      ...
      <div className="rounded-2xl border border-white/[0.06] bg-[#0d0d15] p-5 lg:col-span-2">
        ...
        {visitorsData.length === 0 ? (
          <div className="flex h-[200px] items-center justify-center">
            <p className="text-sm text-white/20">No visitor data yet</p>
          </div>
        ) : (
          <ResponsiveContainer width="100%" height={200}>
            <AreaChart data={visitorsData} margin={{ top: 5, right: 5, left: -20, bottom: 0 }}>
              ...
              <Tooltip content={<CustomTooltip />} />
              <Area type="monotone" dataKey="visitors"  stroke="#C9A96E" strokeWidth={2} fill="url(#goldGrad)" dot={false} activeDot={{ r: 4, fill: '#C9A96E', strokeWidth: 0 }} />
              <Area type="monotone" dataKey="returning" stroke="#4a5568" strokeWidth={2} fill="url(#grayGrad)" dot={false} activeDot={{ r: 4, fill: '#4a5568', strokeWidth: 0 }} />
            </AreaChart>
          </ResponsiveContainer>
        )}
      </div>
      ...
      <div className="rounded-2xl border border-white/[0.06] bg-[#0d0d15] p-5">
        ...
        {cityData.length === 0 ? (
          <div className="flex h-[200px] items-center justify-center">
            <p className="text-sm text-white/20">No property data yet</p>
          </div>
        ) : (
          <ResponsiveContainer width="100%" height={200}>
            <BarChart data={cityData} layout="vertical" margin={{ top: 0, right: 5, left: 0, bottom: 0 }}>
              <XAxis type="number" tick={{ fill: 'rgba(255,255,255,0.25)', fontSize: 10 }} axisLine={false} tickLine={false} />
              <YAxis type="category" dataKey="city" tick={{ fill: 'rgba(255,255,255,0.4)', fontSize: 11 }} axisLine={false} tickLine={false} width={72} />
              <Tooltip content={<CustomTooltip />} />
              <Bar dataKey="listings" fill="#C9A96E" radius={[0, 4, 4, 0]} opacity={0.75} />
            </BarChart>
          </ResponsiveContainer>
        )}
      </div>
