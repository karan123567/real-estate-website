...
const STATUS_STYLES = {
  new:       'bg-green-500/10 text-green-400 border-green-500/20',   // ✅ fixed 'br-' → 'bg-'
...
export default function AdminInquiries() {
  ...
  // ✅ Bug 4 fixed — renamed 'IdleDeadline' to 'id'
  ...
  // ✅ Bug 2 & 3 fixed — all keys correct
  const statusCounts = {
      ...
      <div className="rounded-2xl border border-white/[0.06] bg-[#0d0d15]">
        {loading ? (
          <div className="py-20 text-center">
            <div className="mx-auto h-6 w-6 animate-spin rounded-full
                            border-2 border-white/10 border-t-[#C9A96E]" />
          </div>
              ...
              >
                <div className="flex items-start justify-between gap-4">
                  ...
                  <div className="flex-shrink-0 text-right">
                    ...
                    <p className="mt-1 text-xs text-white/25">{formatDate(inquiry.created_at)}</p>
                  </div>
                </div>
              </div>
            ))}
